...
int main()
{
    ...
    FILE * f1 = fopen ("chirpMono.wav", "wb"); // this is  the file where the one channel sound file will be saved
    FILE * f2 = fopen ("chirpFFT.txt", "w"); // a text file that will have the left channel sound data,
    //just to see what we're dealing with
    FILE * f3 = fopen ("chirpSample.csv", "w");  // an excel file that have the data to use in matLab
    fseek (f , 0 , SEEK_END);
    lSize = ftell (f);
    rewind (f);
